...
def full_disk_format(device, create_boot=False, create_swap=False, swap_size=1024):
    # Criando um arquivo para armazenar o nome do disco
    disk_name = device.path
    with open("/home/rsp/.installer/sourceDiskName.sh", "w") as f:
        f.write(f"export disk_name={disk_name}\n")

    disk_label = ('gpt' if device.getLength('B') > 2**32 * .9 * device.sectorSize  
                  or is_efi_supported()
                  else 'msdos')
    ...
    mkpart = (
        (is_efi_supported(), EFI_MOUNT_POINT, 'vfat', 'mkfs.vfat {} -F 32 ', 300),
        (create_boot, '/boot', 'ext4', 'mkfs.ext4 -F {}', 1024),
        (create_swap, SWAP_MOUNT_POINT, 'swap', 'mkswap {}', swap_size),
        (True, '/', 'ext4', 'mkfs.ext4 -F {}', 0),
    )
    ...
    def run_parted(cmd): 
        return os.system('parted --script --align optimal {} {} ; sync'.format(device.path, cmd))
    ...
    for partition in mkpart:
        if partition[0]:
            ...
            log("Executing: " + mkpart_cmd)
            run_parted(mkpart_cmd)
            partition_path = f"{device.path}{partition_prefix}{partition_number}"
            ...
            while True:
                ...
                if num_tries < 5:
                    num_tries += 1
                    err(f"Could not find {partition_path}, waiting 1s...")
                    os.system("sync")
                    time.sleep(1)
                else:
                    show_error(
            ...
            # Se for a partição raiz "/", salvar o caminho dela
            if partition[1] == '/':
                with open("/home/rsp/.installer/sourceDiskPartition.sh", "w") as f:
                    f.write(f"export disk_partition={partition_path}\n")
